...
go build ${GOFLAGS} github.com/ctylim/certificate-transparency-go-p192/trillian/integration/ct_hammer
...
set +e
./ct_hammer --log_config "${CT_CFG}" --ct_http_servers=${CT_SERVERS} --mmd=30s --testdata_dir=$(go list -f '{{ .Dir }}' github.com/ctylim/certificate-transparency-go-p192)/trillian/testdata --metrics_endpoint="localhost:${metrics_port}" --logtostderr ${HAMMER_OPTS}
